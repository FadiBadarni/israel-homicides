"""Fast 2024 candidate-list preview from Arab48's محليات (Local) listing.

Identical recipe to the 2025 preview — only the date window changes.
Walks listing pages, filters by date + homicide-keyword title, writes CSV.

Output: ``output/arab48_localities_2024_candidates.csv``.
Cost: $0 (no LLM, just HTTP).
"""
...
DATE_TO = "2024-12-31"
# Pages 150-200 cover 2024 (verified earlier). Cap at 220 for headroom
# in case some pages have fewer items than expected.
MAX_PAGES = 220
...
async def main() -> None:
    ...
    out_csv = Path("output/arab48_localities_2024_candidates.csv")
