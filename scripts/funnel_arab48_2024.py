"""3-stage funnel for 2024 backfill from arab48/محليات candidates.

Identical recipe to ``funnel_arab48_2025.py``, only constants differ:
year window, run_id, CSV/audit paths, and the existing-cases dedup
which queries 2024 rows (currently empty — so stage 1's dedup-against-
existing is a no-op for this first run).

Stage 1 — Free deterministic filter
  • Drop URLs already in the DB
  • Regex-extract candidate (name, city) from arab48 titles
  • Soft-flag sentencing/arrest titles
  • Hard-skip when (name, city, month) matches an existing 2024 case
    (none yet on first run, kept for resumability)

Stage 2 — Cheap Gemini Flash batch on remaining titles
Stage 3 — Full pipeline on survivors

Total expected: ~$1, ~1 hour.
"""
...
def _load_existing_year_cases(year: str) -> list[dict]:
    """Returns existing canonical_cases whose incident_date year matches.
    First run for a year returns []; second run picks up its own writes."""
...
async def main() -> None:
    ...
    print(
        ...
        f"--date-from {YEAR}-01-01 --date-to {YEAR}-12-31 --no-narrate"
    )
